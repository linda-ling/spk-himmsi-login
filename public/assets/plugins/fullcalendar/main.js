...
var FullCalendar = (function (exports) {
    ...
    Permission to use, copy, modify, and/or {{ asset('') }}assets/distribute this software for any
    ...
    */
    function expandRecurringRanges(eventDef, duration, framingRange, dateEnv, recurringTypes) {
        ...
        // the recurrence {{ asset('') }}assets/plugins don't guarantee that all-day events are start-of-day, so we have to
        if (eventDef.allDay) {
    ...
    */
    ...
    var BASE_OPTION_REFINERS = {
        ...
        eventDragMin{{ asset('') }}assets/distance: Number,
        expandRows: Boolean,
        height: identity,
        ...
        selectMin{{ asset('') }}assets/distance: Number,
        selectable: Boolean,
    ...
    var BASE_OPTION_DEFAULTS = {
        ...
        longPressDelay: 1000,
        eventDragMin{{ asset('') }}assets/distance: 5,
        expandRows: false,
        navLinks: false,
        selectable: false,
    };
    ...
    */
    ...
    function buildEventSourceMeta(raw, context) {
        ...
        for (var i = defs.length - 1; i >= 0; i -= 1) { // later-added {{ asset('') }}assets/plugins take precedence
            var def = defs[i];
    ...
    */
    ...
    /*
    this array is exposed on the root namespace so that UMD {{ asset('') }}assets/plugins can add to it.
    see the rollup-bundles script.
    */
    var global{{ asset('') }}assets/plugins = [
    ...
    var CalendarDataManager = /** @class */ (function () {
        ...
        CalendarDataManager.prototype.processRawCalendarOptions = function (optionOverrides, dynamicOptionOverrides) {
            ...
            var pluginHooks = this.buildPluginHooks(optionOverrides.{{ asset('') }}assets/plugins || [], global{{ asset('') }}assets/plugins);
            ...
            for (var optionName in raw) {
                if (optionName !== '{{ asset('') }}assets/plugins') { // because {{ asset('') }}assets/plugins is special-cased
        ...
        CalendarDataManager.prototype._computeCurrentViewData = function (viewType, optionsData, optionOverrides, dynamicOptionOverrides) {
            ...
            if (!viewSpec) {
                throw new Error("viewType \"" + viewType + "\" is not available. Please make sure you've loaded all neccessary {{ asset('') }}assets/plugins");
            }
    ...
    */
    ...
    function computeFallbackHeaderFormat(datesRep{{ asset('') }}assets/distinctDays, dayCnt) {
        // if more than one week row, or if there are a lot of columns with not much space,
        // put just the day numbers will be in each cell
        if (!datesRep{{ asset('') }}assets/distinctDays || dayCnt > 10) {
    ...
    var DayHeader = /** @class */ (function (_super) {
        ...
        DayHeader.prototype.render = function () {
            ...
            var _a = this.props, dates = _a.dates, dateProfile = _a.dateProfile, datesRep{{ asset('') }}assets/distinctDays = _a.datesRep{{ asset('') }}assets/distinctDays, renderIntro = _a.renderIntro;
            var dayHeaderFormat = this.createDayHeaderFormatter(context.options.dayHeaderFormat, datesRep{{ asset('') }}assets/distinctDays, dates.length);
            return (createElement(NowTimer, { unit: "day" }, function (nowDate, todayRange) { return (createElement("tr", null,
                renderIntro && renderIntro('day'),
                dates.map(function (date) { return (datesRep{{ asset('') }}assets/distinctDays ? (createElement(TableDateCell, { key: date.toISOString(), date: date, dateProfile: dateProfile, todayRange: todayRange, colCnt: dates.length, dayHeaderFormat: dayHeaderFormat })) : (createElement(TableDowCell, { key: date.getUTCDay(), dow: date.getUTCDay(), dayHeaderFormat: dayHeaderFormat }))); }))); }));
        };
        return DayHeader;
    }(BaseComponent));
    function createDayHeaderFormatter(explicitFormat, datesRep{{ asset('') }}assets/distinctDays, dateCnt) {
        return explicitFormat || computeFallbackHeaderFormat(datesRep{{ asset('') }}assets/distinctDays, dateCnt);
    }
    ...
    */
    ...
    function renderMicroColGroup(cols, shrinkWidth) {
        ...
        HOWEVER, Chrome was getting messing up {{ asset('') }}assets/distributing the width to <td>/<th> elements with colspans.
    ...
    */
    var AutoScroller = /** @class */ (function () {
        ...
        AutoScroller.prototype.handleSide = function (edge, seconds) {
            ...
            var inv{{ asset('') }}assets/distance = edgeThreshold - edge.{{ asset('') }}assets/distance;
            var velocity = // the closer to the edge, the faster we scroll
             ((inv{{ asset('') }}assets/distance * inv{{ asset('') }}assets/distance) / (edgeThreshold * edgeThreshold)) * // quadratic
        ...
        AutoScroller.prototype.computeBestEdge = function (left, top) {
            ...
            for (var _i = 0, _a = this.scrollCaches; _i < _a.length; _i++) {
                ...
                var left{{ asset('') }}assets/dist = left - rect.left;
                var right{{ asset('') }}assets/dist = rect.right - left;
                var top{{ asset('') }}assets/dist = top - rect.top;
                var bottom{{ asset('') }}assets/dist = rect.bottom - top;
                // completely within the rect?
                if (left{{ asset('') }}assets/dist >= 0 && right{{ asset('') }}assets/dist >= 0 && top{{ asset('') }}assets/dist >= 0 && bottom{{ asset('') }}assets/dist >= 0) {
                    if (top{{ asset('') }}assets/dist <= edgeThreshold && this.everMovedUp && scrollCache.canScrollUp() &&
                        (!bestSide || bestSide.{{ asset('') }}assets/distance > top{{ asset('') }}assets/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'top', {{ asset('') }}assets/distance: top{{ asset('') }}assets/dist };
                    }
                    if (bottom{{ asset('') }}assets/dist <= edgeThreshold && this.everMovedDown && scrollCache.canScrollDown() &&
                        (!bestSide || bestSide.{{ asset('') }}assets/distance > bottom{{ asset('') }}assets/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'bottom', {{ asset('') }}assets/distance: bottom{{ asset('') }}assets/dist };
                    }
                    if (left{{ asset('') }}assets/dist <= edgeThreshold && this.everMovedLeft && scrollCache.canScrollLeft() &&
                        (!bestSide || bestSide.{{ asset('') }}assets/distance > left{{ asset('') }}assets/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'left', {{ asset('') }}assets/distance: left{{ asset('') }}assets/dist };
                    }
                    if (right{{ asset('') }}assets/dist <= edgeThreshold && this.everMovedRight && scrollCache.canScrollRight() &&
                        (!bestSide || bestSide.{{ asset('') }}assets/distance > right{{ asset('') }}assets/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'right', {{ asset('') }}assets/distance: right{{ asset('') }}assets/dist };
                    }
                }
            }
            return bestSide;
    ...
    - minimum {{ asset('') }}assets/distance required before dragging
    ...
    */
    var FeaturefulElementDragging = /** @class */ (function (_super) {
        ...
        function FeaturefulElementDragging(containerEl, selector) {
            ...
            _this.delay = null;
            _this.min{{ asset('') }}assets/distance = 0;
            _this.touchScrollAllowed = true; // prevents drag from starting and blocks scrolling during drag
            _this.mirrorNeedsRevert = false;
            _this.isInteracting = false; // is the user validly moving the pointer? lasts until pointerup
            _this.isDragging = false; // is it INTENTFULLY dragging? lasts until after revert animation
            _this.isDelayEnded = false;
            _this.is{{ asset('') }}assets/distanceSurpassed = false;
            ...
            _this.onPointerDown = function (ev) {
                if (!_this.isDragging) { // so new drag doesn't happen while revert animation is going
                    ...
                    if (_this.isInteracting && // not destroyed via pointerdown handler
                        ...
                        _this.startDelay(ev);
                        if (!_this.min{{ asset('') }}assets/distance) {
                            _this.handle{{ asset('') }}assets/distanceSurpassed(ev);
                        }
                    }
                }
            };
            _this.onPointerMove = function (ev) {
                if (_this.isInteracting) {
                    ...
                    if (!_this.is{{ asset('') }}assets/distanceSurpassed) {
                        var min{{ asset('') }}assets/distance = _this.min{{ asset('') }}assets/distance;
                        var {{ asset('') }}assets/distanceSq = void 0; // current {{ asset('') }}assets/distance from the origin, squared
                        var deltaX = ev.deltaX, deltaY = ev.deltaY;
                        {{ asset('') }}assets/distanceSq = deltaX * deltaX + deltaY * deltaY;
                        if ({{ asset('') }}assets/distanceSq >= min{{ asset('') }}assets/distance * min{{ asset('') }}assets/distance) { // use pythagorean theorem
                            _this.handle{{ asset('') }}assets/distanceSurpassed(ev);
                        }
                    }
                    if (_this.isDragging) {
        ...
        FeaturefulElementDragging.prototype.handle{{ asset('') }}assets/distanceSurpassed = function (ev) {
            this.is{{ asset('') }}assets/distanceSurpassed = true;
            this.tryStartDrag(ev);
        };
        FeaturefulElementDragging.prototype.tryStartDrag = function (ev) {
            if (this.isDelayEnded && this.is{{ asset('') }}assets/distanceSurpassed) {
    ...
    */
    var DateSelecting = /** @class */ (function (_super) {
        ...
        function DateSelecting(settings) {
            ...
            dragging.min{{ asset('') }}assets/distance = options.selectMin{{ asset('') }}assets/distance || 0;
    ...
    var EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                dragging.min{{ asset('') }}assets/distance = ev.isTouch ? 0 : options.eventDragMin{{ asset('') }}assets/distance;
                dragging.delay =
    ...
    var EventResizing = /** @class */ (function (_super) {
        ...
        function EventResizing(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                _this.dragging.min{{ asset('') }}assets/distance = component.context.options.eventDragMin{{ asset('') }}assets/distance;
    ...
    */
    var ExternalDraggable = /** @class */ (function () {
        function ExternalDraggable(el, settings) {
            ...
            this.handlePointerDown = function (ev) {
                var dragging = _this.dragging;
                var _a = _this.settings, min{{ asset('') }}assets/distance = _a.min{{ asset('') }}assets/distance, longPressDelay = _a.longPressDelay;
                dragging.min{{ asset('') }}assets/distance =
                    min{{ asset('') }}assets/distance != null ?
                        min{{ asset('') }}assets/distance :
                        (ev.isTouch ? 0 : BASE_OPTION_DEFAULTS.eventDragMin{{ asset('') }}assets/distance);
                dragging.delay =
    ...
    var InferredElementDragging = /** @class */ (function (_super) {
        ...
        function InferredElementDragging(containerEl) {
            ...
            _this.handlePointerDown = function (ev) {
                _this.emitter.trigger('pointerdown', ev);
                if (!_this.shouldIgnoreMove) {
                    // fire dragstart right away. does not support delay or min-{{ asset('') }}assets/distance
                    _this.emitter.trigger('dragstart', ev);
                }
            };
    ...
    */
    ...
    var DayTableView = /** @class */ (function (_super) {
        ...
        DayTableView.prototype.render = function () {
            ...
            var headerContent = options.dayHeaders && (createElement(DayHeader, { ref: this.headerRef, dateProfile: props.dateProfile, dates: dayTableModel.headerDates, datesRep{{ asset('') }}assets/distinctDays: dayTableModel.rowCnt === 1 }));
    ...
    */
    ...
    var DayTimeColsView = /** @class */ (function (_super) {
        ...
        DayTimeColsView.prototype.render = function () {
            ...
            var headerContent = options.dayHeaders && (createElement(DayHeader, { dates: dayTableModel.headerDates, dateProfile: dateProfile, datesRep{{ asset('') }}assets/distinctDays: true, renderIntro: hasAttachedAxis ? this.renderHeadAxis : null }));
    ...
    */
    ...
    global{{ asset('') }}assets/plugins.push(interactionPlugin, dayGridPlugin, timeGridPlugin, listPlugin, plugin, googleCalendarPlugin);
    ...
    exports.global{{ asset('') }}assets/plugins = global{{ asset('') }}assets/plugins;
