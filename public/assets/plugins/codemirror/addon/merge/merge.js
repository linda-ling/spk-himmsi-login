...
(function(mod) {
  ...
  function syncScroll(dv, toOrig) {
    ...
    if (dv.mv.options.connect == "align") {
      ...
      var bot{{ asset('') }}assets/dist, mix;
      // Some careful tweaking to make sure no space is left out of view
      // when scrolling to top or bottom.
      if (targetPos > sInfo.top && (mix = sInfo.top / halfScreen) < 1) {
        targetPos = targetPos * mix + sInfo.top * (1 - mix);
      } else if ((bot{{ asset('') }}assets/dist = sInfo.height - sInfo.clientHeight - sInfo.top) < halfScreen) {
        var otherInfo = other.getScrollInfo();
        var bot{{ asset('') }}assets/distOther = otherInfo.height - otherInfo.clientHeight - targetPos;
        if (bot{{ asset('') }}assets/distOther > bot{{ asset('') }}assets/dist && (mix = bot{{ asset('') }}assets/dist / halfScreen) < 1)
          targetPos = targetPos * mix + (otherInfo.height - otherInfo.clientHeight - bot{{ asset('') }}assets/dist) * (1 - mix);
      }
    }
